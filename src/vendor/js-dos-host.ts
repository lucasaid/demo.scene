...
class DosHost {
  ...
  public global = typeof window !== "undefined" ? (window as any) : {}
  ...
  constructor() {
    ...
    try {
      if (
        ...
        const wmodule = new WebAssembly.Module(
          Uint8Array.of(0x0, 0x61, 0x73, 0x6d, 0x01, 0x00, 0x00, 0x00)
        )
        if (wmodule instanceof WebAssembly.Module) {
          this.wasmSupported =
            new WebAssembly.Instance(wmodule) instanceof WebAssembly.Instance
        }
      }
    } catch (error) {
  ...
  private compileDosBox(url: string, cache: ICache, module: DosModule) {
    ...
    if (this.wasmSupported && isWasmUrl) {
      ...
      if (module.log) {
        ...
        module.log(
          "[DEBUG] Wasm supported: " + this.wasmSupported + ", url: " + url
        )
      }
        ...
        progress: (total, loaded) => {
          if (module.onprogress) {
            module.onprogress(
              "Resolving DosBox (" + url + ")",
              buildTotal,
              Math.min(Build.wasmSize, loaded)
            )
          }
        },
        fail: (url: string, status: number, message: string) => {
